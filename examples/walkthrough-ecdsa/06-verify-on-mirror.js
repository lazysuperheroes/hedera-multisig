#!/usr/bin/env node
/**
 * Walkthrough step 6 (ECDSA): verify a transaction on the mirror node.
 *
 * Identical to `examples/walkthrough-dapp/06-verify-on-mirror.js` —
 * duplicated here so this walkthrough is fully self-contained.
 *
 * Pass the transaction ID printed by the coordinator after step 5. The
 * script polls the mirror node (with retry/backoff) until it confirms
 * the transaction was externalized, then prints the consensus timestamp,
 * fee, and HBAR transfers.
 *
 * Usage:
 *   node 06-verify-on-mirror.js 0.0.X@1234567890.000000000
 */
...
async function main() {
  console.log(chalk.bold.cyan('\n━━━ Verify ECDSA ceremony on mirror node ━━━\n'));

  const txId = process.argv[2];
  if (!txId) {
    console.error(chalk.red('❌ Usage: node 06-verify-on-mirror.js <transactionId>'));
    console.error(chalk.gray('   Example: node 06-verify-on-mirror.js 0.0.1234@1764452239.277675395'));
    process.exit(1);
  }
  ...
  console.log(chalk.bold.green('✅ Mirror confirmed (ECDSA ceremony succeeded):'));
  ...
  console.log(chalk.gray('\n   ECDSA validation complete: a 2-of-3 KeyList of secp256k1 keys was'));
  console.log(chalk.gray('   accepted by the consensus layer the same as Ed25519 would have been.'));
  console.log(chalk.gray('   Multi-sig on Hedera is genuinely key-type-agnostic.\n'));
}
