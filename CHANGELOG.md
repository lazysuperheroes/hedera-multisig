...
- **`SCHEDULE_ANNOUNCE` / `SCHEDULE_CREATED` protocol seam.** Coordinator-only client message; server stamps `session.mode = 'scheduled'` + persists `scheduleId` / `scheduleExpirationTime` / `scheduleMemo` / `payerAccountId` / `adminKey` / `innerTxDetails` / `innerTxBase64`, then broadcasts `SCHEDULE_CREATED` to all participants. Late joiners get the same context inline via `AUTH_SUCCESS.sessionInfo`. Architectural decision documented in `SKILLS.md`: server is signaling channel only; mirror node is the source of truth for schedule signature state — the server **does not** broker `ScheduleSignTransaction` submissions. (`shared/protocol.js`, `server/WebSocketServer.js`, `server/SigningSessionManager.js`, `test/schedule-announce.test.js`)
- **`ScheduledReview` browser component** — dedicated review surface for HIP-423 schedules on `/session/<id>`, replacing `TransactionReview` when the session is in scheduled mode. No 120s countdown; renders inner-tx details + "expires in 23h 47m" framing + live mirror-node lookup confirming the schedule exists on-chain plus signature progression. Approve calls the wallet's `executeWithSigner` with a `ScheduleSignTransaction`; reject broadcasts to the session without touching the chain. (`dapp/components/ScheduledReview.tsx`, `dapp/app/session/[id]/page.tsx`, `dapp/lib/mirror-node.ts:fetchScheduleInfo`)
- **CLI participant native scheduled-tx handling.** `client/SigningClient.js` emits `scheduleCreated` events; `cli/commands/participant.js` adds an inline review/sign flow that prompts `YES` (or auto-approves with `--yes`), pre-checks `OPERATOR_ID`/`OPERATOR_KEY` env vars **before** prompting, then submits `ScheduleSignTransaction` via `ScheduledWorkflow.signSchedule`. Distinguishes three post-sign cases: "your sig met threshold → inner tx X executed", "sig accepted, more needed", "schedule already executed by another signer". Late joiners hit the same flow via `setImmediate`. Removes the awkward "join via dApp → drop to CLI to call `schedule sign`" hand-off.
- **`examples/walkthrough-ecdsa/`** — sixth walkthrough: 2-of-3 ceremony with secp256k1 ECDSA keys (vs the Ed25519 default) on the dApp + CLI hybrid path. Validates the entire stack — coordinator, dApp, CLI participant, mirror — is genuinely curve-agnostic in a real ceremony, not just in the unit-tested mixed-type code path. Aborts at keygen if `PrivateKey.generateECDSA()` ever silently falls back to a different curve (DER-prefix sanity check).
- **`examples/walkthrough-scheduled/`** — fifth walkthrough demonstrating async multi-sig via HIP-423 scheduled transactions over a 24h+ window. Sibling to `walkthrough-dapp`: same keys, same threshold account, hybrid HashPack + CLI signers; expected runtime ~25 minutes, ~6 ℏ. Includes `01-status.js` helper that maps signatory pubkeys back to alice/bob/carol names from local state. README contrasts realtime vs scheduled side-by-side.
- **`ScheduledLaunched` ShareStep variant** — when the session is in scheduled mode, `/create`'s share view renders a dedicated "Schedule launched" panel (HashScan link, expiration in `in 23h 47m` form, prominent "Build another transaction" CTA) instead of the realtime signing/completed/failed views. Fixes the previous behavior where the coordinator's view stayed in 'signing' phase indefinitely after announcing a schedule. (`dapp/components/create/ShareStep.tsx`, `dapp/components/create/SessionMonitor.tsx`)
- **Multi-node freeze (canonical Hedera multi-sig pattern)** — wire protocol restored to send `signatures: string[]` (one base64 sig per `SignedTransaction` body). `selectNodeAccountIds()` helper in `shared/node-selection.js` (Node) + `dapp/lib/node-selection.ts` (browser); default = random subset of 6 nodes for CLI (single-node default for the dApp because the WalletConnect `SignTransaction` RPC signs one body per popup; multi-node would mean N popups), well within Hedera's 6 KB tx-size cap. Strategy override: `'subset' | 'all' | 'specific'`.
...
### Changed
- **`ScheduledWorkflow.signSchedule` returns richer execution metadata.** Reads `receipt.scheduledTransactionId` to set `executed: true` + `innerTxId` when *this* signature met the threshold (distinct from the existing `alreadyExecuted: true` branch returned for `SCHEDULE_ALREADY_EXECUTED`, which means another signer triggered first). Lets callers differentiate "your sig met threshold" from "sig accepted, more needed" from "no-op".
...
- **`/session/[id]` "No session information found" after `/join` redirect.** React 18 StrictMode's dev double-mount could destroy the sessionStorage credential handoff before AUTH_SUCCESS landed a `reconnectionToken` in localStorage — leaving mount #2 with no PIN (sessionStorage already consumed) and no surface-able saved session (`useSessionRecovery` discards saved sessions without a `reconnectionToken`). Removed the eager `sessionStorage.removeItem('hedera-multisig-pending-join')` consume; the per-tab handoff now persists for the tab's lifetime, making the read idempotent across remounts and across retry-on-auth-failure. Production also benefits — previously a failed AUTH left the user with no recovery path. (`dapp/app/session/[id]/page.tsx`)
- **Documentation cleanup post-fix.** Updated CLAUDE.md, README.md, ROADMAP.md, walkthrough-contract README + scripts, walkthrough-token README, in-product error message at `/session/[id]`, and header comments in `dapp/lib/diagnose-body-mismatch.ts`, `dapp/lib/probe-verify-paths.ts`, `dapp/lib/node-selection.ts`, `shared/node-selection.js` to reflect the corrected diagnosis: the bug was upstream in `@hashgraph/hedera-wallet-connect`, not in the wallets themselves. Single-node freeze remains the dApp default for a different reason now (WC RPC signs one body per popup; multi-node would mean N popups). New `docs/UPSTREAM_ISSUE_WALLET_CONNECT.md` ready to file with the Hedera Hashgraph wallet-connect maintainers. Bump dapp 2.1.39 → 2.1.40.
- **Multi-sig `ContractExecuteTransaction` now works for wallet signers (HashPack, Kabila).** Upstream issue filed at [hashgraph/hedera-wallet-connect#694](https://github.com/hashgraph/hedera-wallet-connect/issues/694). Root cause: `@hashgraph/hedera-wallet-connect`'s `DAppSigner.signTransaction` rebuilds a fresh `TransactionBody` via `transactionToTransactionBody(transaction, nodeAccountId)` and sends THAT to the wallet for signing — then reattaches the wallet's signature to the *original* preserved `signedTx.bodyBytes`. For HBAR / token transfers the rebuild happens to produce byte-identical proto bytes (verify works); for `ContractExecuteTransaction` the rebuild diverges (likely default-value handling for `transactionFee` or proto field-ordering in `contractCall`), so the signature is over content the dApp never sees. Multi-sig aggregation breaks. We bypassed the SDK helper entirely: our `signTransaction` now decodes the outer Transaction proto via `@hashgraph/proto`, extracts the original bodyBytes from `SignedTransaction`, calls `signer.request({method: HederaJsonRpcMethod.SignTransaction, params: {signerAccountId, transactionBody: base64(originalBodyBytes)}})` directly, and reattaches the resulting signature to a fresh `SignedTransaction` wrapping the same original bodyBytes. Verify works. Aggregation works. Wallet signers are now functional for `ContractExecuteTransaction` live ceremonies, closing the largest gap in v2.2's wallet support. Verified empirically on testnet: same contract that previously failed identically across HashPack and Kabila now signs and verifies for both. Diagnostic helpers (`dapp/lib/diagnose-body-mismatch.ts`, `dapp/lib/probe-verify-paths.ts`) retained behind localhost gating for future investigation. (`dapp/lib/walletconnect.ts`)
