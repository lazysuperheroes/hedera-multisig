...
- **Multi-node freeze (canonical Hedera multi-sig pattern)** — wire protocol restored to send `signatures: string[]` (one base64 sig per `SignedTransaction` body). `selectNodeAccountIds()` helper in `shared/node-selection.js` (Node) + `dapp/lib/node-selection.ts` (browser); default = random subset of 6 nodes (single-node freeze for the dApp due to HashPack re-freezing `ContractExecuteTransaction`), well within Hedera's 6 KB tx-size cap. Strategy override: `'subset' | 'all' | 'specific'`.
